...
fn postfix_sequence(parts: &Vec<Gregexp>, buffer: &mut Vec<GregexpSegment>) {
...
fn postfix_any(gregexp: &Gregexp, buffer: &mut Vec<GregexpSegment>) {
    match gregexp {
        Gregexp::Sequence(parts) => postfix_sequence(parts, buffer),
        Gregexp::Group(expr, modifier) => postfix_group(expr, modifier, buffer),
        Gregexp::CharacterGroup(_) => todo!(),
        Gregexp::ExactMatch(char, modifer) => postfix_exact(*char, modifer, buffer),
    }
}

pub fn postfix(gregexp: &Gregexp) -> Vec<GregexpSegment> {
    let mut buffer = vec![];
...
pub fn postfix_to_string(postfix : &Vec<GregexpSegment>) -> String {
    ...
    for segment in postfix {
        match segment {
            ...
            GregexpSegment::CountModifier(modifier) => {
                match modifier {
